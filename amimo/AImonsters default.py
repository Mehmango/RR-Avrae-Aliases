...
This module contains the basic behavior MONSTERS for general use and some monster MONSTERS. This is far from a comprehensive list, just a starting point!
Use this module as a template when creating your own monster AI.

* THINGS YOUR MODULE WILL NEED TO BE COMPATIBLE:
* - Your monster MONSTERS should have the same required parameters as the goblin function below, as well as **kwargs.
...
def goblin(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, **kwargs):
    """
    ...
    * Let me know if corrections are needed
    ...
    viableTiles: A dict representing all viable tiles. Keys are in the form of alphanumeric coordinates (eg: "A1")
    ...
    scale: The scale that will be applied to the values calculated for each coordinate by this function. Useful for adjusting the function's sensitivity to sub-MONSTERS
    ...
    **kwargs: For future support
    ...
    viableTiles = basic(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, speed=speed)
    viableTiles = rangedAttacker(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, scale=100, speed=speed, meleeRange=meleeRange, rangedRange=rangedRange)
    # viableTiles = meleeAttacker(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, scale=100, meleeRange=meleeRange)
    
    return viableTiles
                
def avoidStandingInSpaceWithCombatants(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, scale=1, **kwargs):
            ...
            for ax in range(x - selfSizeMultiplier + 1, x + otherSizeMultiplier):
                for ay in range(y - selfSizeMultiplier + 1, y + otherSizeMultiplier):
                    if ax >= 0 and ax < len(mapGrid) and ay >= 0 and ay < len(mapGrid[0]):
                        viableTiles.pop(convertCoordinates((ax, ay)), None)
    
    return viableTiles
    
def meleeAttacker(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, scale=1, meleeRange=5, **kwargs):
        ...
        if not combatant.monster_name and 'location' in combatantInfo[combatant.name]:  # Combatant is a player
            ...
            for ax in range(playerX - monsterSizeMultiplier, playerX + 1 + playerSizeMultiplier):
                for ay in range(playerY - monsterSizeMultiplier + 1, playerY + 1 + playerSizeMultiplier):
                    if ax >= 0 and ax < len(mapGrid) and ay >= 0 and ay < len(mapGrid[0]):
                        viableTiles[convertCoordinates((ax, ay))] += 1*scale
                    
    return viableTiles

def rangedAttacker(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, scale=1, rangedRange=30, **kwargs):
    allPlayerNames = []
    for combatant in combat().combatants:
        if not combatant.monster_name and 'location' in combatantInfo[combatant.name]:  # Combatant is a player
            allPlayerNames.append(combatant.name)
    for tile, value in viableTiles.items():
        totalValue = 0
        for playerName in allPlayerNames:
            distance = getDistance(tile, playerName)
            totalValue += distance/TILE_SIZE if distance <= rangedRange else 0
        viableTiles[tile] += totalValue/len(allPlayerNames)*scale
    
    return viableTiles

def basic(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance, scale=1, speed=30, **kwargs):
    viableTiles = avoidStandingInSpaceWithCombatants(mapInfo, combatantInfo, mapGrid, viableTiles, convertCoordinates, getDistance)
